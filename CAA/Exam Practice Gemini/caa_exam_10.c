...
int main() {
    ...
    double C[MAX_N * MAX_N]; // Едномерен масив (достатъчно голям за всички елементи)
    int N;                   // Размерност
    ...
    // 5. Обработка а): Образуване на масив C (Елементи ПОД главния диагонал)
    createArrayC(A, N, C, &sizeC);

    // Отпечатване на резултата от а)
    if (sizeC > 0) {
        printf("\n--- Резултати ---\n");
        printArray(C, sizeC, "а) Масив C (Елементи под главния диагонал):");
        ...
        printf("\nНяма елементи под главния диагонал (N е твърде малко).\n");
    }
...
void printTaskInfo() {
    ...
    printf("   намиращи се ПОД главния диагонал.\n");
...
// Логика за подточка а) - Под главния диагонал
void createArrayC(double matrix[MAX_N][MAX_N], int n, double arr[], int *size) {
    *size = 0; // Нулиране на брояча
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            // Условие за под главния диагонал: индекс на ред > индекс на колона
            if (i > j) {
                arr[*size] = matrix[i][j];
                (*size)++;
            }
