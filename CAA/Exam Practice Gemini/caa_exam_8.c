...
void calculateColSums(double matrix[MAX_N][MAX_N], int n, double arr[]);
...
int main() {
    ...
    double C[MAX_N];        // Едномерен масив за сумите от колоните
    int N;                  // Размерност
    ...
    // 5. Обработка а): Образуване на масив C (Сума от всяка КОЛОНА)
    calculateColSums(A, N, C);

    // Отпечатване на резултата от а)
    printf("\n--- Резултати ---\n");
    printArray(C, N, "а) Масив C (Суми на елементите по КОЛОНИ):");
...
void printTaskInfo() {
    ...
    printf("   елементите от всяка КОЛОНА на А.\n");
...
// Логика за подточка а) - Сумиране по КОЛОНИ
void calculateColSums(double matrix[MAX_N][MAX_N], int n, double arr[]) {
    // Външният цикъл върти колоните (j)
    for (int j = 0; j < n; j++) {
        double currentSum = 0;
        // Вътрешният цикъл върти редовете (i) за текущата колона
        for (int i = 0; i < n; i++) {
            currentSum += matrix[i][j];
        }
        arr[j] = currentSum; // Записваме сумата в масив C на индекс j
    }
}
